  ...
  */
        ...
        var _prepareInstance = function _prepareInstance(fragment) {
          ...
          while (partIndex < parts.length && node !== null) {
            ...
            if (!isTemplatePartActive(_part3)) {
              ...
              if (_part3.type === 'node') {
                var _part4 = _this5.processor.handleTextExpression(_this5.options);

                _part4.insertAfterNode(node.previousSibling);

                _this5._parts.push(_part4);
              } else {
                var _this5$_parts;

                (_this5$_parts = _this5._parts).push.apply(_this5$_parts, babelHelpers.toConsumableArray(_this5.processor.handleAttributeExpressions(node, _part3.name, _part3.strings, _this5.options)));
              }
  ...
  function (_AttributeCommitter) {
    ...
    function PropertyCommitter(element, name, strings) {
      ...
      _this6 = babelHelpers.possibleConstructorReturn(this, babelHelpers.getPrototypeOf(PropertyCommitter).call(this, element, name, strings));
  ...
  var prepareTemplateStyles = function prepareTemplateStyles(renderedDOM, template, scopeName) {
    ...
    if (window.ShadyCSS.nativeShadow) {
      ...
      var _style2 = template.element.content.querySelector('style');

      renderedDOM.insertBefore(_style2.cloneNode(true), renderedDOM.firstChild);
    } else {
      ...
      value: function update(changedProperties) {
        ...
        if (this._needsShimAdoptedStyleSheets) {
          ...
          this.constructor._uniqueStyles.forEach(function (s) {
            ...
            _this8.renderRoot.appendChild(style);
          });
        }
      }
      /**
      ...
      get: function get() {
        ...
        if (this._styles === undefined) {
          ...
          styleSet.forEach(function (v) {
            return _this9._styles.unshift(v);
          });
        }
  ...
  var repeat = directive(function (items, keyFnOrTemplate, template) {
    ...
    return function (containerPart) {
      if (!babelHelpers.instanceof(containerPart, NodePart)) {
      ...
      var oldKeys = keyListCache.get(containerPart) || []; // New part list will be built up as we go (either reused from old parts
      // or created for new keys in this update). This is saved in the above
      // cache at the end of the update.

      var newParts = []; // New value list is eagerly generated from items along with a parallel
      ...
      var newHead = 0;
      var newTail = newValues.length - 1; // Overview of O(n) reconciliation algorithm (general approach based on
  ...
  _exports.$ToDoItemStyles = ToDoItemStyles;

  var ToDoItem =
  /*#__PURE__*/
  function (_LitElement) {
    babelHelpers.inherits(ToDoItem, _LitElement);

    function ToDoItem() {
      babelHelpers.classCallCheck(this, ToDoItem);
      return babelHelpers.possibleConstructorReturn(this, babelHelpers.getPrototypeOf(ToDoItem).apply(this, arguments));
    }
  ...
  _exports.ToDoItem = ToDoItem;
  customElements.define('to-do-item', ToDoItem);
  var ToDoItem$1 = {
    ToDoItem: ToDoItem
  };
  _exports.$ToDoItem = ToDoItem$1;

  var ToDo =
  /*#__PURE__*/
  function (_LitElement2) {
    babelHelpers.inherits(ToDo, _LitElement2);
    babelHelpers.createClass(ToDo, null, [{
      key: "properties",
    ...
    function ToDo() {
      ...
      _this10 = babelHelpers.possibleConstructorReturn(this, babelHelpers.getPrototypeOf(ToDo).call(this));
      _this10.list = [_this10.todoItem('clean the house'), _this10.todoItem('buy milk')];
      _this10.todo = '';
      _this10.createNewToDoItem = _this10.createNewToDoItem.bind(babelHelpers.assertThisInitialized(babelHelpers.assertThisInitialized(_this10)));
      _this10.handleKeyPress = _this10.handleKeyPress.bind(babelHelpers.assertThisInitialized(babelHelpers.assertThisInitialized(_this10)));
      _this10.handleInput = _this10.handleInput.bind(babelHelpers.assertThisInitialized(babelHelpers.assertThisInitialized(_this10)));
      return _this10;
    }

    babelHelpers.createClass(ToDo, [{
      key: "todoItem",
      value: function todoItem(todo) {
        return {
          todo: todo
        };
      }
      ...
      value: function createNewToDoItem() {
        this.list = [].concat(babelHelpers.toConsumableArray(this.list), [this.todoItem(this.todo)]);
        this.todo = '';
      }
    }, {
      key: "handleKeyPress",
      ...
      key: "deleteItem",
      value: function deleteItem(indexToDelete) {
        this.list = this.list.filter(function (toDo, index) {
